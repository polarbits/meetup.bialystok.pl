...
const debug = (0, _debug2.default)('core:data');
...
class Data extends _queryBuilder2.default {
  ...
  _batchBodyBuilder(body) {
    ...
    const path = `/${apiVersion}${this.url().split(apiVersion)[1].split('?')[0]}`;
  ...
  async resolveRelatedModels(result) {
    ...
    const resolvers = this.relationships.map(async reference => {
      ...
      const references = resultsToProcess.filter(row => row[reference]).map(row => {
      ...
      if (target === 'user') {
        load._url = `${this._getInstanceURL(this.instance.instanceName)}/users/`;
      }
  ...
  async loadNextPage(response, objects) {
    ...
    if (hasNextPageMeta && hasNotEnoughResults) {
      ...
      const nextParams = _querystring2.default.parse(response.next.replace(/.*\?/, ''));
      const q = _querystring2.default.stringify(_extends({}, this.query, nextParams));
  ...
  async request(url) {
    ...
    try {
      ...
      err.message = (0, _lodash4.default)(err, 'response.data.query');
      throw err;
    }
  }
  ...
  async list() {
    debug('list');
    const self = this;
    const urls = [this.url()].concat(this.queries.map(query => (this._query.query = query) && this.url()));
    debug('urls', urls);
    const uniqueIds = [];
  ...
  _replaceCustomTypesWithValue(items) {
    if (Array.isArray(items)) {
      return items.map(item => Object.keys(item).reduce((all, key) => _extends({}, all, {
        [key]: this._replaceCustomType(key, item)
      }), {}));
    }

    return Object.keys(items).reduce((all, key) => _extends({}, all, {
  ...
  _mapFieldsForSingleItem(item, fields) {
    return Object.keys(fields).reduce((all, key) => (0, _lodash2.default)(all, fields[key] || key, (0, _lodash4.default)(item, key)), {});
  }
  ...
  firstOrCreate(attributes, values = {}) {
    ...
    return this.where(query).firstOrFail().catch(() => this.create((0, _lodash6.default)(attributes, values)));
  }
  ...
  async updateOrCreate(attributes, values = {}) {
    ...
    try {
      ...
      return this.create((0, _lodash6.default)(attributes, values));
    }
  }
  ...
  where(column, operator, value) {
    ...
    if (Array.isArray(column)) {
      column.map(([itemColumn, itemOperator, itemValue]) => this.where(itemColumn, itemOperator, itemValue));
    ...
    const nextQuery = column.split('.').reverse().reduce((child, item) => ({
      [item]: child === null ? {
  ...
  whereBetween(column, min, max) {
    return this.where([[column, 'gte', min], [column, 'lte', max]]);
  }
  ...
  _chunk(items, size) {
    return items.map((e, i) => i % size === 0 ? items.slice(i, i + size) : null).filter(Boolean);
  }

  async _batch(body, headers) {
    const type = Array.isArray(body[0]) ? 'PATCH' : isNaN(body[0]) === false ? 'DELETE' : 'POST';
      ...
      async function next() {
        ...
        try {
          if (request) {
            ...
            const items = data.map(item => item.content ? item.content : item);
  ...
  create(body) {
    ...
    return this.fetch(fetchObject.url, fetchObject, headers).then(this.resolveRelatedModels.bind(this)).then(this._replaceCustomTypesWithValue.bind(this)).then(this._mapFields.bind(this));
  }
  ...
  update(id, body) {
    ...
    if (isQueryUpdate) {
      return this.list().then(items => {
        ...
        return this._batch(ids).then(this.resolveRelatedModels.bind(this)).then(this._replaceCustomTypesWithValue.bind(this)).then(this._mapFields.bind(this));
      });
    }
    ...
    if (body instanceof _formData2.default) {
      ...
      return this._batch(id, headers).then(this.resolveRelatedModels.bind(this)).then(this._replaceCustomTypesWithValue.bind(this)).then(this._mapFields.bind(this));
    } else {
      fetchObject.body = JSON.stringify(body);
    }
